...
def optimize(inputFile):
    ...
    for wk in worker.index:
        for w in wip.index:
            SX.loc[w,wk] = 0.5/(wip.loc[w,'time']*(1-worker.loc[wk,w]*0.05))
    
    # Optimization 1: get 24h plan
    mod=grb.Model()
    ...
    # Optimizaiton 2: get short-term plan
    mod=grb.Model()
    ...
    for m in M:
        ...
        for wk in WK:
            ...
            mod.addConstr((sum(BX[w,wk]*wip.loc[w,m]*SX.loc[w,wk] for w in W for wk in WK) - np.ceil(material.loc['per second',m]))*TX[wk]\
                          <= material.loc['in stock',m])
            # check stock + change_rate * time <= capacity
            mod.addConstr(np.ceil(material.loc['per second',m]) - sum(BX[w,wk]*wip.loc[w,m]*SX.loc[w,wk] for w in W for wk in WK)*TX[wk]\
    ...
    for w in W:
        ...
        for p in P:
            ...
            mod.addConstr((sum(BY[p,wk]*product.loc[w,p]/product.loc['time',p] for p in P for wk in WK) - sum(BX[w,wk]*wip.loc[w,m]*SX.loc[w,wk] for wk in WK))*TY[wk]\
                          <= wip.loc[w,'in stock'])
            mod.addConstr((sum(BY[p,wk]*product.loc[w,p]/product.loc['time',p] for p in P for wk in WK) - sum(BX[w,wk]*wip.loc[w,m]*SX.loc[w,wk] for wk in WK))*TX[wk]\
                          <= wip.loc[w,'in stock'])
            # check stock + change_rate * time <= capacity
    ...
    for wk in WK:
        ...
        mod.addConstr(sum(X[w,wk]/SX.loc[w,wk] for w in W) + sum(Y[p,wk]*product.loc['time',p] for p in P) <= R)

        # one task per worker in given time frame
        mod.addConstr(sum(BX[w,wk] for w in W) + sum(BY[p,wk] for p in P) <= 1)

        mod.addConstr(sum(X[w,wk]/SX.loc[w,wk] for w in W) <= TX[wk])
        mod.addConstr(sum(Y[p,wk]/product.loc['time',p] for p in P) <= TY[wk])
        for w in W:
    ...
    for wk in WK:
        for w in W:
            if BX[w,wk].x != 0:
                ...
                soln_next.loc[wk,'units'] = X[w,wk].x
                
        for p in P:
            if BY[p,wk].x != 0:
                soln_next.loc[wk,'行为'] = p
                soln_next.loc[wk,'units'] = Y[p,wk].x
