...
def optimize(inputFile):
    ...
    for w in W:
        mod.addConstr(sum(product.loc[w,p]*Y[p,wk] for p in P for wk in WK) <= X[w,wk] + wip.loc[w,'in stock'])
        mod.addConstr(X[w,wk]  >= 0)

    for p in P:
        mod.addConstr(Y[p,wk] >= 0)
    for w in WK:
        mod.addConstr(sum(product.loc['time',p]*Y[p,wk] for p in P)              + sum(wip.loc[w,'time']*X[w,wk] for w in W) <= total_hour*3600)  

    # total time limit
    mod.addConstr(sum(product.loc['time',p]*Y[p,wk] for p in P for wk in WK)              + sum(wip.loc[w,'time']*X[w,wk] for w in W for wk in WK) <= total_hour*3600*N)
    ...
    # set objective
    efficient_time = sum(X[w,wk]*wip.loc[w,'time'] for w in W for wk in WK)                  + sum(Y[p,wk]*product.loc['time',p]*worker.loc[wk,p]/10 for p in P for wk in WK)
    ...
    for m in M:
        ...
        for wk in WK:
            ...
            mod.addConstr((sum(BX[w,wk]*wip.loc[w,m] for w in W for wk in WK) - np.floor(material.loc['per hour',m]))*TX[wk]                      <= material.loc['in stock',m])
            # check stock + change_rate * time <= capacity
            mod.addConstr((np.floor(material.loc['per hour',m]) - sum(BX[w,wk]*wip.loc[w,m] for w in W for wk in WK))*TX[wk]                              <= (material.loc['capacity',m] - material.loc['in stock',m]))
            mod.addConstr((np.floor(material.loc['per hour',m]) - sum(BX[w,wk]*wip.loc[w,m] for w in W for wk in WK))*R                             <= (material.loc['capacity',m] - material.loc['in stock',m]))
    ...
    for w in W:
        ...
        for wk in WK:
            ...
            mod.addConstr((sum(BY[p,wk]*product.loc[w,p]/product.loc['time',p] for p in P for wk in WK) - sum(BX[w,wk]*wip.loc[w,m]*SX.loc[w,wk] for wk in WK))*TY[wk]                      <= wip.loc[w,'in stock'])
            mod.addConstr((sum(BY[p,wk]*product.loc[w,p]/product.loc['time',p] for p in P for wk in WK) - sum(BX[w,wk]*wip.loc[w,m]*SX.loc[w,wk] for wk in WK))*TX[wk]                      <= wip.loc[w,'in stock'])
            # check stock + change_rate * time <= capacity
            mod.addConstr(wip.loc[w,'in stock'] + (sum(BX[w,wk]*wip.loc[w,m]*SX.loc[w,wk] for wk in WK) - sum(BY[p,wk]*product.loc[w,p]/product.loc['time',p] for p in P for wk in WK))*TY[wk]  <= wip.loc[w,'capacity'])
            mod.addConstr(wip.loc[w,'in stock'] + (sum(BX[w,wk]*wip.loc[w,m]*SX.loc[w,wk] for wk in WK) - sum(BY[p,wk]*product.loc[w,p]/product.loc['time',p] for p in P for wk in WK))*TX[wk]  <= wip.loc[w,'capacity'])
    ...
    for wk in WK:
        ...
        #mod.addConstr(sum(X[w,wk]*wip.loc[w,'time']/0.5*(1-worker.loc[wk,w]*0.05) for w in W)\
                      #+ sum(Y[p,wk]*product.loc['time',p] for p in P) >= T*B - 3600)
    ...
    soln_wip.to_excel(writer, sheet_name='24H制造安排')
    soln_product.to_excel(writer, sheet_name='24H委派安排')
